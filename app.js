...
function renderDashboard() {
  ...
  var cardsHtml = [
    ...
    { label: 'MQTT主题设备', num: comDevices, change: '共 ' + comDevices + ' 台IoT设备', cls: 'info' }
  ].map(function(c) {
...
function renderAssetTable() {
  ...
  var html = filtered.map(function(a) {
    var mqttTopicHtml = a.mqttTopic ? '<span class="mqtt-badge">' + escapeHtml(a.mqttTopic) + '</span>' : '<span style="color:#556677">-</span>';
    return '<tr>' +
...
function testMqttDevice(id) {
  ...
  showToast('正在测试 ' + a.mqttTopic + ' MQTT通讯...', 'info');
  setTimeout(function() {
    var success = Math.random() > 0.3;
    if (success) {
      showToast(a.mqttTopic + ' MQTT通讯正常，响应: OK', 'success');
      addActivity('MQTT检测', a.name + ' ' + a.mqttTopic + ' MQTT测试通过');
    } else {
      showToast(a.mqttTopic + ' MQTT通讯失败，请检查连接', 'error');
      addActivity('MQTT检测', a.name + ' ' + a.mqttTopic + ' MQTT测试失败');
    }
    saveData();
  }, 1200);
}
...
function checkMqttDevices() {
  ...
  setTimeout(function() {
    ...
    var msg = 'MQTT设备发现：找到 ' + found.length + ' 个在线设备 (' + found.map(function(r) { return r.topic; }).join(', ') + ')';
    showToast(msg, 'success');
    addActivity('MQTT检测', msg);
  ...
  data.assets.forEach(function(a) {
    if (a.lat && a.lng) {
      ...
      var popupContent = '<b>' + escapeHtml(a.name) + '</b><br>型号: ' + escapeHtml(a.model) +
        ...
        '<br>MQTT设备: ' + (a.mqttTopic || '无') + '<br>位置: ' + escapeHtml(a.location);
      L.circleMarker([parseFloat(a.lat), parseFloat(a.lng)], {
...
function renderOnlineTable() {
  ...
  var html = onlineAssets.map(function(a) {
    ...
    return '<tr>' +
      ...
      '<td>' + (a.mqttTopic ? '<span class="mqtt-badge">' + escapeHtml(a.mqttTopic) + '</span>' : '-') + '</td>' +
...
function renderCommandHistory() {
  ...
  var html = data.commandHistory.slice().reverse().map(function(h) {
    ...
    return '<tr>' +
      ...
      '<td>' + (h.mqttTopic ? '<span class="mqtt-badge">' + escapeHtml(h.mqttTopic) + '</span>' : '-') + '</td>' +
...
function sendCommand(cmd) {
  ...
  setTimeout(function() {
    ...
    addActivity('MQTT指令', 'Pub devices/' + mqttTopic + '/cmd ' + cmd + ' -> ' + device.name + ' [' + result + ']');
    saveData();
    renderCommandHistory();
  }, 1500);
}
